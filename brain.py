...
def brain1(state: State, p = False):
...
def brain3(state: State, p = True):
    if state.left_wall() > 0:
        ...
        if action == -1:
            return brain1(state, p)
        return copy.deepcopy(state).next(action)
    else :
        return brain1(state, p)
